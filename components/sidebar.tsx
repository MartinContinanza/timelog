  ...
  return (
    <Link href={href} style={{ textDecoration: 'none' }}>
      <div style={{
        ...
        color: active ? 'var(--cu-dark)' : 'var(--text2)',
        fontSize: 13.5, fontWeight: active ? 500 : 400,
        background: active ? 'var(--cu-light)' : 'transparent',
    ...
    <aside style={{
      ...
      <div style={{ padding: '20px 20px 16px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 32, height: 32, background: 'var(--cu)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      <div style={{ padding: 12, borderTop: '1px solid var(--border)' }}>
        <form action={logout}>
          <button
            type="submit"
            title="Cerrar sesión"
            style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 8, borderRadius: 8, cursor: 'pointer', transition: 'background .15s', border: 'none', background: 'transparent', width: '100%', textAlign: 'left' }}
            onMouseEnter={e => (e.currentTarget.style.background = 'var(--bg2)')}
            onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
          >
            <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--cu)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 600, flexShrink: 0 }}>
              {initials}
            </div>
            <div>
              <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)' }}>{fullName}</div>
              {sector && <div style={{ fontSize: 11, color: 'var(--text3)' }}>{sector}</div>}
            </div>
          </button>
        </form>
      </div>
