...
export default async function HistorialPage() {
  ...
  // Current month entries with account info
  const now = new Date()
  const periodStart = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-01`

  const { data: entriesData } = await supabase
    .from('entries')
    .select('*, account:accounts(id, code, name, group_name)')
    .eq('employee_id', employee?.id ?? '')
    .gte('date', periodStart)
    .order('date', { ascending: false })

  type EntryWithAccount = EntryRow & { account: Pick<AccountRow, 'id' | 'code' | 'name' | 'group_name'> | null }
  const entries = (entriesData ?? []) as EntryWithAccount[]

  // All accounts for the edit modal select
  const { data: accsData } = await supabase.from('accounts').select('id, code, name, group_name').eq('active', true).order('code')
  const accounts = (accsData ?? []) as Pick<AccountRow, 'id' | 'code' | 'name' | 'group_name'>[]
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 12, color: 'var(--text3)' }}>
            {entries.length} registros · <strong style={{ color: 'var(--text)' }}>{totalHours}hs</strong> imputadas
          </span>
        </div>
      </div>
