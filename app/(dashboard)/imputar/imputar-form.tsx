...
function MiniCalendar({
  year, month, hoursMap, selectedDate, onDayClick,
}: {
    ...
    <>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 2, marginBottom: 4 }}>
        {['L','M','X','J','V','S','D'].map((l, i) => (
          <div key={l} style={{ textAlign: 'center', fontSize: 10, fontWeight: 600, color: 'var(--text3)', padding: '3px 0' }}>
            {l}
          </div>
        ))}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 2 }}>
        {cells.map((d, i) => {
          ...
          if (isWeekend)          { bg = 'var(--bg2)'; textColor = 'var(--text3)' }
          if (hrs && hrs >= 8)    { bg = 'var(--cu-pale)'; borderColor = '#bae6fd' }
          else if (hrs)           { bg = '#fffbeb'; borderColor = '#fde68a' }
          if (isToday)            borderColor = 'var(--cu)'
          if (isSelected && !isWeekend) { bg = 'var(--cu-light)'; borderColor = 'var(--cu)' }
          if (isFuture)           textColor = 'var(--text3)'
          ...
          return (
            <div
              ...
              style={{
                ...
                color: isToday ? 'var(--cu)' : textColor,
                background: bg, border: `1.5px solid ${borderColor}`,
                cursor: isFuture || isWeekend ? 'default' : 'pointer',
                opacity: isFuture ? 0.4 : 1, transition: 'all .15s', padding: '2px 1px',
              }}
            >
              {d}
              {hrs ? (
                <span style={{
                  ...
                  background: hrs >= 8 ? 'var(--cu)' : 'var(--warning)',
                  color: 'white', borderRadius: 3, padding: '0 3px', marginTop: 1,
                }}>{hrs}h</span>
              ) : null}
            </div>
          )
...
export default function ImputarForm({ accounts, hoursMap, totalHours, hoursTarget, year, month, monthName }: Props) {
  ...
  const [activityType, setActivityType] = useState('Auditoría')
  ...
  const remaining = Math.max(hoursTarget - totalHours, 0)
  const circ      = 175.9
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        ...
        const textarea = formRef.current?.querySelector<HTMLTextAreaElement>('textarea[name="comment"]')
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20, alignItems: 'start' }}>
      ...
      <div>
        ...
        <div style={{ display: 'flex', background: 'var(--bg2)', borderRadius: 8, padding: 3, gap: 2, marginBottom: 14 }}>
          {modeBtn('dia',    'Por día')}
          {modeBtn('rango',  'Rango de fechas')}
          {modeBtn('semana', 'Semana completa')}
        </div>
        ...
        <form ref={formRef} onSubmit={handleSubmit}>
          <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 20, boxShadow: 'var(--shadow)' }}>
            ...
            <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
              <svg viewBox="0 0 24 24" fill="none" stroke="var(--cu)" strokeWidth="2" width="16" height="16">
            ...
            {mode === 'dia' && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                  ...
                  <input
                    type="date" name="date" required className="form-input"
                    max={todayStr} value={dateValue}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                  <label style={{ fontSize: 12, fontWeight: 500, color: 'var(--text2)' }}>
                    Horas <span style={{ color: 'var(--danger)' }}>*</span>
                  </label>
                  <input type="number" name="hours" required className="form-input" min="0.5" max="12" step="0.5" defaultValue="8" />
                </div>
              </div>
            )}
            ...
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--text2)', marginBottom: 5 }}>
                Subcuenta <span style={{ color: 'var(--danger)' }}>*</span>
              </label>
              ...
              <span style={{ fontSize: 11, color: 'var(--text3)', marginTop: 3, display: 'block' }}>Formato: SECTOR-PROGRAMA-CLIENTE</span>
            </div>

            {/* TIPO DE ACTIVIDAD */}
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--text2)', marginBottom: 6 }}>
                Tipo de actividad <span style={{ color: 'var(--danger)' }}>*</span>
              </label>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {ACTIVITY_TYPES.map(type => (
                  <button key={type} type="button" onClick={() => setActivityType(type)} style={{
                    padding: '5px 12px', borderRadius: 20, fontSize: 12, fontWeight: 500,
                    border: `1.5px solid ${activityType === type ? 'var(--cu)' : 'var(--border2)'}`,
                    background: activityType === type ? 'var(--cu)' : 'var(--bg)',
                    color: activityType === type ? 'white' : 'var(--text2)',
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', paddingTop: 8 }}>
              <button type="reset" className="btn" onClick={() => { setActivityType('Auditoría'); setDateValue(todayStr) }}>
                Limpiar
              </button>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 20, boxShadow: 'var(--shadow)' }}>
          ...
          <MiniCalendar
            year={year} month={month} hoursMap={hoursMap}
            selectedDate={dateValue}
            onDayClick={date => { setDateValue(date); setMode('dia') }}
          />
        ...
        <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '18px 20px', boxShadow: 'var(--shadow)' }}>
          <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--text3)', letterSpacing: '0.4px', textTransform: 'uppercase', marginBottom: 10 }}>
            Progreso del mes
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
              <svg width="70" height="70" viewBox="0 0 70 70" style={{ transform: 'rotate(-90deg)' }}>
                ...
                <circle
                  cx="35" cy="35" r="28" fill="none" stroke="var(--cu)" strokeWidth="6"
            ...
            <div>
              <div style={{ fontSize: 22, fontWeight: 600, color: 'var(--text)' }}>
                {totalHours}<span style={{ fontSize: 13, color: 'var(--text3)' }}>/{hoursTarget}hs</span>
              </div>
              <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 2 }}>{remaining}hs restantes</div>
              {remaining > 0 && (
                <div style={{ fontSize: 12, color: 'var(--text3)' }}>
                  ~{(remaining / Math.max(1, 11)).toFixed(1)}hs/día
                </div>
              )}
            </div>
          </div>
