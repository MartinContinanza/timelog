...
interface Props {
  period: string; monthLabel: string
  totalHours: number; hoursTarget: number
  missingDays: number; approverName: string
  currentClosure: MonthlyClosureRow | null
  pastClosures: ClosureWithApprover[]
  closeDay: number
}

export default function RevisionClient({ period, monthLabel, totalHours, hoursTarget, missingDays, approverName, currentClosure, pastClosures, closeDay }: Props) {
  ...
  function handleSubmit() {
    startTr(async () => {
      ...
      else { toast(r.success!, 'success'); setSubmitted(true) }
    })
  }
  ...
  return (
    <>
      {/* Panel */}
      <div style={{ background: 'linear-gradient(135deg, var(--cu-pale) 0%, #f0f9ff 100%)', border: '1px solid #bae6fd', borderRadius: 'var(--radius-lg)', padding: 24, textAlign: 'center', marginBottom: 24 }}>
        {submitted ? (
          <>
            <div style={{ width: 48, height: 48, background: 'var(--success)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
              <svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" width="24" height="24"><polyline points="20 6 9 17 4 12"/></svg>
            </div>
            <h3 style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)', marginBottom: 6, textTransform: 'capitalize' }}>{monthLabel} enviado a revisión</h3>
          ...
          <>
            <h3 style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)', marginBottom: 6, textTransform: 'capitalize' }}>Cierre de {monthLabel}</h3>
            ...
            <div style={{ textAlign: 'left', maxWidth: 320, margin: '0 auto 20px' }}>
              ...
              <CheckItem ok={daysOk}  text={daysOk ? 'Sin días faltantes ✓' : `${missingDays} día${missingDays > 1 ? 's' : ''} sin imputación`} />
              <CheckItem ok={approverOk} text={`Aprobador: ${approverName}`} />
              <CheckItem ok={true} text={`Cierre: ${closeDay} de ${new Date().toLocaleDateString('es-AR', { month: 'long' })}`} />
            </div>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
              <a href="/imputar" className="btn">Completar horas primero</a>
              <button onClick={handleSubmit} className="btn btn-primary" disabled={isPending}>
                {isPending ? 'Enviando…' : (
                  <><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" width="15" height="15"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>{hoursOk && daysOk ? 'Enviar a revisión' : 'Enviar de todas formas'}</>
                )}
              </button>
            </div>
          </>
      ...
      {pastClosures.length > 0 && (
        <>
          <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 12 }}>Historial de cierres</div>
          <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow)', overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr>{['Período','Enviado','Aprobador','Estado'].map(h => (
              ...
              <tbody>
                {pastClosures.map(c => (
                  <tr key={c.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '11px 16px' }}><span className={`badge ${STATUS_BADGE[c.status] ?? 'badge-gray'}`}>{STATUS_LABEL[c.status] ?? c.status}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
