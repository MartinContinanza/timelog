...
export default async function RevisionPage() {
  ...
  // Past closures with approver name
  ...
  const pastClosures = (pastData ?? []) as ClosureWithApprover[]

  // Approver name
  let approverName = 'Sin aprobador asignado'
    ...
    <>
      <div style={{ background: 'var(--bg)', borderBottom: '1px solid var(--border)', padding: '14px 28px', display: 'flex', alignItems: 'center', position: 'sticky', top: 0, zIndex: 10 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 1, textTransform: 'capitalize' }}>Cierre mensual · {monthLabel}</div>
        </div>
      </div>
